...
- 用户确认只补授权、不补证据；高影响候选必须由真实实验边界承载，缺证时保持现状并给项目证据支持的补证路径，不发明环境或替代方案。
...
- 当前全量 on：GPT-5.6-sol / high、Claude Opus 5、Grok 4.5 与 Kimi K3 均为 144/144。
- 配对 A/B on / off 与净增：GPT 96/84（+12）、Opus 96/77（+19）、Grok 96/69（+27）、K3 96/79（+17）；runner token on 均高于 off，不把质量增益表述为无条件省 token。
