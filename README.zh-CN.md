...
最近冻结结果（2026-07-31）包含 18 条全量现实委托和其中 12 条配对 A/B，只对 [`docs/evaluation-results.md`](docs/evaluation-results.md) 保留的指纹成立。只有 2 题是明确低风险对照；其余只给自然症状、意见或宽泛请求，关键事实藏在项目代码、日志、brief、diff、任务状态和 runbook 中，覆盖错误前提、有害修法、模糊需求、参考与复用边界、可信文档、长期记忆、长任务恢复和生产边界。
...
五组完整配对结果的加权分均为正增益，但形态不同：GPT 与 Opus 四层均增益；Grok 主要增益于 judgment；Gemini 四层均增益且本次 runner token 更低；K3 增益较小且 boundary 高 2 分。单轮结果不能证明 odai 必然增费或省费，应按模型实测判断，不把它包装成无条件的免费升级。旧指纹成绩不迁移，未完成的模型结果不进入最终表。
