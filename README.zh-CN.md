...
六组完整配对结果的加权分均为正增益，但形态不同：GPT、Opus 与 D4F 四层均增益；Grok 主要增益于 judgment；Gemini 四层均增益且本次 runner token 更低；K3 增益较小且 boundary 高 2 分。单轮结果不能证明 odai 必然增费或省费，应按模型实测判断，不把它包装成无条件的免费升级。skill、题本或评分语义变化时成绩不迁移；不改变行为的 harness I/O 修复不使完整证据失效，未完成的模型结果不进入最终表。
