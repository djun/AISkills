...
当前候选基线（2026-08-06）包含 19 条全量现实委托和其中 13 条配对 A/B。只有 2 题是明确低风险对照；其余只给自然症状、意见或宽泛请求，关键事实藏在项目代码、日志、brief、diff、任务状态和 runbook 中。
...
四个 runner 的全量 on 与 A/B on 均达到满分；A/B 净增为 12–27 分，最大增量仍在 judgment。四个 runner 的 on token 都更高，因此当前证据支持“按任务用额外成本换治理、专业判断与验收质量”，不支持无条件省 token。
