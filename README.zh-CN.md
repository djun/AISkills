...
四组完整配对结果的加权分均为正增益，但形态不同：GPT 四层均增益，Grok 主要增益来自 judgment，Gemini 四层均增益且本次 runner token 更低，K3 增益较小且 boundary 高 2 分。单轮结果不能证明 odai 必然增费或省费，应按模型实测判断，不把它包装成无条件的免费升级。旧指纹成绩不迁移，未完成的模型结果不进入最终表。
