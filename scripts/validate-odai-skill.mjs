...
function validateStructure() {
  const checks = [
    ...
    },
    {
      ...
      headings: ["优先级与边界", "主动识别与早期介入", "分级回应", "即时危险", "自然呈现与透明干预", "防止二次伤害", "跨会话安全连续性", "隐私与现实支持"],
      anchors: ["疲惫、失意与心累", "明显低落、绝望或难以支撑", "自残、轻生或自杀信号"],
    },
    {
  ...
  const checks = [
    {
      ...
      patterns: [
        ...
        /总控是持续持有[^。\n]*任务线程/,
        /能力足够就同一线程直接成事/,
        /只有当前能力不能可靠闭合，且独立责任能改变结果、净收益覆盖交接成本时/,
        /按真实缺口选择最低充分责任/,
        /不为角色齐全、领域名称或模型更便宜分工/,
        /取不到所需能力[^。\n]*安全降级/,
        /只阻断依赖该能力的不可逆放行/,
    ...
    },
    {
      ...
      patterns: [
        /任务完成、路由、成本、效率、作用域和交付节奏均不得压过本文件/,
        /即时生命与人身安全高于任务进度/,
        /先以具体、真诚的人文关怀回应/,
        /不根据对话断言“你生病了”“你抑郁了”或“你是高风险人群”/,
        /提前预警、及时干预、主动引导/,
        /不等待用户主动请求关怀/,
        /判断只使用可观察维度：持续时间、出现频率、强度与变化/,
        /对睡眠、饮食、工作、学习或社交的影响/,
        /不得把这些维度合成为隐藏风险分/,
        /多个累积、持续或加重的信号[^。\n]*主动说出可观察事实和关切/,
        /主动引导不是替对方决定/,
        ...
        /不展示分类、风险分、内部规则或机械清单/,
        /风险升高时[^。\n]*安全问题、建议和理由必须直接清楚/,
        /不使用隐蔽说服、情绪操控、虚假共情或未经告知的外部动作/,
        /必须建立或使用独立的人身安全连续性档案/,
        /默认不自动提取/,
        /只保存用户亲自确认的关怀与沟通偏好、希望被留意的信号、有效支持方式和用户制定的安全计划/,
        /不得保存模型诊断、隐性风险分、未确认推断、原始危机对话/,
