...
开放任务允许多种合理方案。judge 不奖励内部路由、固定格式、支撑文件读取或额外仪式，只判断真实结果、来源忠实度、可执行性、验证和边界；只有题目明确观察已安装专业能力的自动发现时，才把对应 skill 的真实读取与最终整合作为产品行为判断，不能因“调用过”本身加分。确定性门只覆盖客观真值、改动范围和副作用。
