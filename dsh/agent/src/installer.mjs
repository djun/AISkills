...
export function renderAgentCompositionForDsh(composition, dshVersion = SUPPORTED_DSH_VERSION) {
  ...
  rendered = replaceCompositionContract(
    rendered,
    OPTIONAL_PROVIDER_COMMENTS[SOURCE_DSH_VERSION],
    targetComment,
...
export async function inspectAgentInstallation(options = {}) {
...
export async function installAgentPreset(options = {}) {
  const presetId = assertPresetId(options.presetId ?? DEFAULT_PRESET_ID);
  const dshHome = resolveDshHome(options.dshHome);
...
export async function uninstallAgentPreset(options = {}) {
...
async function assertPresetIsNotDefault(dshHome, presetId) {
  ...
  try {
    ...
    throw new Error(`cannot verify the default agent preset in ${settingsPath}: ${error.message}`);
  }
  if (settings?.["agent-presets"]?.default === presetId) {
    throw new Error(`refusing to uninstall the default preset ${presetId}; select another agent-presets.default first`);
  }
}

async function inspectTarget(target, presetId) {
  ...
  try {
    ...
    return { status: "drifted", issues: [`managed manifest is unavailable: ${error.message}`] };
  }
...
function validateManifest(manifest, presetId) {
  const issues = [];
  if (manifest?.schemaVersion !== 1) issues.push("unsupported managed manifest schema");
  if (manifest?.package !== packageMetadata.name) issues.push("managed manifest package mismatch");
  if (manifest?.presetId !== presetId) issues.push("managed manifest preset id mismatch");
...
async function tightenTree(root) {
...
async function hashTree(root, options = {}) {
  const found = {};
  await walk(root, "");
