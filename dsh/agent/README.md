...
The installer checks `dsh -V` and currently accepts exactly `0.1.0-rc.6`, `0.1.0-rc.7`, or `0.1.0-rc.8`. It records the detected version in the managed manifest and renders that release's exact Standard composition; any other developer-preview version fails closed.
...
In supported DSH rc.6, rc.7, and rc.8 releases, the one-shot `--profile headless` driver creates a global agent directly and neither mounts nor accepts a session Agent preset. Automated Agent runs must create a Web session with `agentPreset: odai`; use the profile-wide Plugin for one-shot headless tasks.
