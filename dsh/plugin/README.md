...
- configurable compaction calls: summaries inherit the conversation provider/model by default, a user may persist a separate explicit target, same-route summaries inherit the current reasoning effort, cross-model targets remove only an effort matching the durable route, and retention stays at the provider default unless explicitly configured;
