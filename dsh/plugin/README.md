...
- task-state responsibility gaps, inline same-model planner reuse, authorized planner-to-executor continuation, complete current reviewer evidence with controller-local incomplete-packet fallback, formal model-route preflight, and buffered compaction fallback without partial-output contamination.
...
The package accepts exactly `@deepseek-ai/dsh@0.1.0-rc.7`, `0.1.1-rc.1`, or `0.1.1-rc.2`; `0.2.8` supports neither rc.6 nor rc.8, and DSH remains a developer preview, so every added release requires isolated real-load verification before this range expands.
