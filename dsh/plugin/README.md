...
- `auto` (default): ordinary work stays on the configured controller. A narrowly matched multi-source decision blocker may first use one configured read-only researcher child; a complete contextual or explicit planning gap upgrades that same controller turn when its planner mapping is configured. A substantial interface-production gap can likewise upgrade the current turn through an explicit frontend mapping. Multi-surface work and explicit responsive, interaction, visual, product-comprehension, or browser-acceptance requirements contribute auditable signals; a narrow component or copy fix stays direct and records why it remained below the specialist threshold. Reviewer starts an independent child only when a complete hash-addressed evidence packet exists; otherwise its configured route is a read-only same-turn check that cannot claim independent acceptance.
