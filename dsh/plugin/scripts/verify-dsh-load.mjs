...
const wrapper = [
  ...
  "  const compactionProbe = compactionTool.execute({ action: 'set', provider: 'probe-provider', model: 'probe-summary', reasoningEffort: 'high' }, { agent: controller }).then(() => {",
  `    const stored = JSON.parse(readFileSync(${JSON.stringify(compactionConfigPath)}, 'utf8'));`,
  "    if (stored.target?.provider !== 'probe-provider' || stored.target?.model !== 'probe-summary' || stored.target?.reasoningEffort !== 'high') throw new Error(`compaction config was not persisted: ${JSON.stringify(stored)}`);",
  "  });",
  ...
  "    );",
  "  });",
  "  Promise.all([guardsReady, configProbe, sourceProbe, evolutionProbe, outputProbe, compactionProbe, memoryProbe]).then(() => {",
