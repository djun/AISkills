...
| --- | --- | --- | --- |
| `0.2.3` | `0.2.3` | `0.1.0-rc.6`、`0.1.0-rc.7`、`0.1.0-rc.8` | 当前版本；Agent 以 rc.8 Standard 为 source，并为 rc.7/rc.6 精确向后渲染 |
...
2. “支持的 DSH”是精确白名单。例如 `0.2.3` 不自动承诺兼容 rc.5 或 rc.9。
...
机器可读的单一事实源是 [`compatibility.json`](./compatibility.json)。发布前校验器会强制当前双包的 `peerDependencies` 与对应矩阵行完全一致；本文是供人查阅的展开视图。
