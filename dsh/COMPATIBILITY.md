...
| --- | --- | --- | --- |
| `0.2.6` | `0.2.6` | `0.1.0-rc.7`、`0.1.1-rc.1` | 当前候选版本；DSH Runtime、Plugin、Agent、测试与发布工具完整 strict TypeScript 迁移，职责拆分与 0.1.1 兼容 |
...
2. “支持的 DSH”是精确白名单。例如 `0.2.6` 不支持 rc.6，也不自动承诺兼容 `0.1.1-rc.2`。
...
DSH 兼容关系的机器可读单一事实源是 [`compatibility.json`](./compatibility.json)；仓库自有版本规范的事实源是 [`../version-policy.json`](../version-policy.json)。发布前校验器会同时强制版本规范、当前双包 `peerDependencies` 与对应矩阵行完全一致；本文是供人查阅的展开视图。
