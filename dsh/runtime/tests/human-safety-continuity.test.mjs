...
function harness() {
  ...
  const tool = createHumanSafetyContinuityTool({
    storePath,
    directUserTextFor(agent) { return agent.currentText; },
    onChanged(_agent, data) { changes.push(data); },
  });
  const agent = { currentText: "", session: { header: {} } };
...
test("continuity requires explicit current-user persistence and exact user-authored values", async (t) => {
  ...
  assert.throws(() => state.tool.execute({
    ...
    value: "我今天觉得心累。",
  }, { agent: state.agent }), /explicit request/u);
  ...
  for (const refusal of [
    ...
    assert.throws(() => state.tool.execute({
      ...
      category: "care-preference",
      value,
    }, { agent: state.agent }), /explicit(?: affirmative)? request/u);
  ...
  const added = await state.tool.execute({ action: "add", category: "care-preference", value }, { agent: state.agent });
  ...
  assert.throws(() => state.tool.execute({
    ...
    value: "联系一个可信任的人",
  }, { agent: state.agent }), /byte-for-byte/u);
});
...
test("continuity rejects ordinary memory content even when the requested category is safety-scoped", async (t) => {
  ...
  for (const [request, value] of [
    ...
    assert.throws(() => state.tool.execute({
      ...
      category: "care-preference",
      value,
    }, { agent: state.agent }), /human-safety or care content/u);
  }
  assert.equal(readHumanSafetyContinuityStore(state.storePath).entries.length, 0);
...
test("continuity is visible, correctable, exportable, and physically clearable across tool instances", async (t) => {
  ...
  const added = await state.tool.execute({ action: "add", category: "care-preference", value: original }, { agent: state.agent });
  ...
  const shown = await nextSessionTool.execute({ action: "show" }, { agent: state.agent });
  ...
  const exported = await nextSessionTool.execute({ action: "export" }, { agent: state.agent });
  ...
  const cleared = await nextSessionTool.execute({ action: "clear" }, { agent: state.agent });
...
test("continuity rejects child access and private contact data", async (t) => {
  ...
  }, { agent: state.agent }), /contact details/u);

  const child = { currentText: "查看我的安全档案", session: { header: { origin: "subagent" } } };
  assert.throws(() => state.tool.execute({ action: "show" }, { agent: child }), /child agents/u);
});
