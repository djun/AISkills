...
test("compaction target validates explicit model options and applies only to compaction", () => {
  ...
  const sessions = {
    get(sessionId) {
      ...
      return { requestHeader: () => ({ config: { provider: "openai", model: "gpt-5.6-sol", reasoningEffort: "xhigh" } }) };
    },
  };
...
test("configured compaction targets append one strict state protocol without touching inherited or ordinary calls", () => {
  ...
  const originalMessages = [{ id: "stock", role: "user", content: [{ type: "text", text: "Stock compaction instruction" }] }];
