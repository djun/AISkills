...
function responsibilityGapEvent(responsibility, overrides = {}) {
  const marker = ({ researcher: "a", planner: "b", executor: "c", reviewer: "d", frontend: "e", user: "f" })[responsibility];
  return {
...
test("controller output policy is default-concise, turn-stable, request-bounded, and isolated from children", async () => {
  ...
  assert.equal(
    events.find((event) => event.type === "odai/output-budget-applied" && event.data.step === 2).data.budgetSource,
...
test("ordinary turns keep Odai system and tool overhead below the measured pre-adaptive baseline", async () => {
  ...
  const section = (name) => assembled.sections.find((candidate) => candidate.name === name).text;
  ...
  const systemText = assembled.sections.map((candidate) => candidate.text).filter(Boolean).join("\n\n");
  const activeNames = new Set(activeOdaiToolNames(classifyContextActivation(text)));
  const activeTools = ctx.captured.tools.filter((tool) => activeNames.has(tool.name));
...
test("plugin registers canonical prompt, monotonic guard, audit observer, and router", async () => {
  ...
  const tools = new Map(ctx.captured.tools.map((tool) => [tool.name, tool]));
...
test("explicit safety continuity persists across controller sessions without reaching child prompts", async () => {
  ...
  const controllerAgent = {
    session: {
      ...
      append(type, data) { controllerEvents.push({ type, seq: controllerEvents.length + 1, data }); },
    },
  };
  ...
  const childContinuity = childAssembled.sections.find((section) => section.name === "odai:human-safety-continuity").text;
  ...
  await nextCtx.captured.tools.find((tool) => tool.name === "odai_human_safety_continuity")
...
test("configured auto mode upgrades the current controller turn without a child", async () => {
  ...
  assert.match(result.messages[1].content[0].text, /requested controller route: openai\/gpt-5\.6-sol \(reasoning: high, maxTokens: 2048\)/u);
  assert.deepEqual(events.map((event) => event.type), [
    "odai/route-decided",
    "odai/route-context",
  ...
  assert.match(events[1].data.digest, /^[a-f0-9]{64}$/u);
  ...
  assert.deepEqual(events.find((event) => event.type === "odai/output-budget-overridden").data, {
    turn: 1,
    step: 1,
    responsibility: "planner",
  ...
  assert.equal(events.findLast((event) => event.type === "odai/responsibility-scope-stopped").data.reason, "terminal-response");
  ctx.captured.handlers.get("session/event")(agent.session, {
    type: "turn/end",
  ...
  assert.equal(events.findLast((event) => event.type === "odai/responsibility-scope-stopped").data.reason, "direct-user-input");
...
test("resume restores every in-place responsibility's base route without reviving ownership", async () => {
  ...
  for (const role of ["planner", "reviewer", "executor", "frontend"]) {
    ...
    assert.equal(
      events.findLast((event) => event.type === "odai/responsibility-scope-stopped").data.reason,
      "runtime-resume",
      role,
    );
    ...
    const restoration = events.findLast((event) => event.type === "odai/responsibility-scope-restored").data;
...
test("a positionless late header cannot erase durable base-route restoration", async () => {
  ...
  const mismatch = events.find((event) => event.type === "odai/responsibility-scope-restored").data;
  ...
  const restored = events.findLast((event) => event.type === "odai/responsibility-scope-restored").data;
...
test("same-turn deterministic route validation removes the exact persisted mapping and restores the controller", async () => {
  ...
  assert.equal(events.find((event) => event.type === "odai/route-health").data.status, "invalid");
  assert.equal(events.find((event) => event.type === "odai/route-fallback").data.fallbackUsed, true);
...
test("transient same-turn route validation falls back without deleting the persisted mapping", async () => {
  ...
  assert.equal(events.find((event) => event.type === "odai/route-health").data.status, "unhealthy");
});
...
test("a provider failure after route preflight retries the original controller once and preserves recoverable config", async () => {
  ...
  assert.equal(events.findLast((event) => event.type === "odai/responsibility-scope-stopped").data.reason, "route-request-failed");
...
test("an authorized planner card and executor gap continue automatically without another user prompt", async () => {
  ...
  const routeCardTool = ctx.captured.tools.find((tool) => tool.name === "odai_route_card");
  ...
  const interruptedScope = events.find((event) => event.type === "odai/responsibility-scope-started").data;
  assert.equal(events.find((event) => event.type === "odai/route-applied").data.routeCardId, frozen.card.id);
  ctx.captured.handlers.get("session/event")(agent.session, {
    type: "assistant/chunk",
  ...
  const interruption = events.find((event) => event.type === "odai/responsibility-interrupted").data;
  ...
  const resumedScope = events.findLast((event) => event.type === "odai/responsibility-scope-started").data;
  ...
  assert.equal(events.findLast((event) => event.type === "odai/route-card-claimed").data.reason, "output-limit-continuation");
  ...
  assert.equal(events.find((event) => event.type === "odai/responsibility-interruption-consumed").data.resumedScopeId, resumedScope.scopeId);
});
...
test("an executor provider failure after an applied receipt releases the route card for retry", async () => {
  ...
  const failedScope = events.find((event) => event.type === "odai/responsibility-scope-started").data;
  const failedReceipt = events.findLast((event) => event.type === "odai/route-fallback").data;
...
test("reviewer starts a child only from a complete hash-addressed evidence packet", async () => {
  ...
  const events = [
    ...
    ...nativeToolEvents("diff-1", "git diff -- dsh/runtime/src/router.mjs", "diff --git a/router.mjs b/router.mjs\n+bounded change", { callSeq: 101 }),
    ...nativeToolEvents("test-1", "node --test dsh/runtime/tests/router.test.mjs", "tests 14 pass 14 fail 0 exit code: 0", { callSeq: 111 }),
  ];
  const agent = { session: { header: {}, events, append(type, data) { events.push({ type, data }); } } };
  const result = await ctx.captured.handlers.get("agent/pre-step")(
    { agent, turn: 1, step: 1, signal: new AbortController().signal },
  ...
  const mismatchEvents = [
    ...
    { type: "user/message", data: userMessage("实现请求：保持默认行为并修复路由。") },
    { type: "assistant/message", data: { content: [{ type: "text", text: "验收条件 A1：目标测试通过；A2：只修改目标模块。" }] } },
    ...nativeToolEvents("diff-mismatch", "git diff -- dsh/runtime/src/router.mjs", "diff --git a/router.mjs b/router.mjs\n+bounded change", { callSeq: 121 }),
    ...nativeToolEvents("test-mismatch", "node --test dsh/runtime/tests/router.test.mjs", "tests 14 pass 14 fail 0 exit code: 0", { callSeq: 131 }),
  ];
  const mismatchAgent = {
    session: {
      header: {},
  ...
  const fallbackEvents = [responsibilityGapEvent("reviewer")];
  const fallbackAgent = {
    session: {
      header: {},
  ...
  assert.match(fallback.messages[1].content[0].text, /Remain on the current controller route/u);
  assert.match(fallback.messages[1].content[0].text, /continue the authorized task/u);
  assert.match(fallback.messages[1].content[0].text, /not independent acceptance/u);
  ...
  const fallbackDecision = fallbackEvents.find((event) => event.type === "odai/route-decided").data;
  ...
  const fallbackResult = fallbackEvents.find((event) => event.type === "odai/route-result").data;
  ...
  const executeEvents = [responsibilityGapEvent("reviewer")];
  const executeAgent = {
    session: {
      header: {},
  ...
  const failedEvents = [
    ...
    ...nativeToolEvents("diff-2", "git diff -- dsh/runtime/src/router.mjs", "diff --git a/router.mjs b/router.mjs\n+bounded change", { callSeq: 141 }),
    ...nativeToolEvents("test-2", "node --test dsh/runtime/tests/router.test.mjs", "tests 14 pass 13 fail 1 exit code: 1", { callSeq: 151, isError: true }),
  ];
  ...
  const failedContext = failedEvents.find((event) => event.type === "odai/route-context").data;
  ...
  assert.equal(failedEvents.find((event) => event.type === "odai/route-result").data.independent, false);
});
...
test("an unavailable frontend mapping falls back locally with an explicit non-receipt notice", async () => {
  ...
  assert.match(result.messages[1].content[0].text, /Continue locally as the current controller/u);
  assert.match(result.messages[1].content[0].text, /Do not claim the configured frontend responsibility ran/u);
  assert.match(result.messages[1].content[0].text, /Canonical craft reference/u);
  assert.equal(events.some((event) => event.type === "odai/route-upgrade"), false);
  assert.equal(events.find((event) => event.type === "odai/route-result").data.status, "fallback");
...
test("frontend incident upgrades in place, verifies its actual route, and overrides the global ceiling", async () => {
  ...
  const startedScope = events.find((event) => event.type === "odai/responsibility-scope-started").data;
  ...
  const renderedRoute = routingTool.output.render({}, shown)[0].text;
  ...
  assert.equal(events.findLast((event) => event.type === "odai/route-decided").data.action, "direct");
  ...
  assert.equal(events.find((event) => event.type === "odai/responsibility-interruption-preserved").data.scopeId, startedScope.scopeId);
  ...
  const resumedDecision = events.findLast((event) => event.type === "odai/route-decided").data;
  ...
  assert.equal(events.find((event) => event.type === "odai/responsibility-interruption-resume-requested").data.scopeId, startedScope.scopeId);
  assert.match(resumed.messages[1].content[0].text, /RESPONSIBILITY_OUTPUT_LIMIT_CONTINUATION/u);
  const resumedScope = events.findLast((event) => event.type === "odai/responsibility-scope-started").data;
  ...
  const consumed = events.find((event) => event.type === "odai/responsibility-interruption-consumed").data;
  assert.equal(consumed.scopeId, startedScope.scopeId);
  assert.equal(consumed.resumedScopeId, resumedScope.scopeId);
  assert.equal(events.findLast((event) => event.type === "odai/route-applied").data.responsibilityScopeId, resumedScope.scopeId);
});
...
test("a new authenticated task clears preserved interruption even when a plugin notice follows it", async () => {
  ...
  assert.equal(events.find((event) => event.type === "odai/responsibility-interruption-cleared").data.scopeId, "scope-old-frontend");
...
test("frontend missing mapping falls through and an omitted role budget keeps the global ceiling", async () => {
  ...
  assert.deepEqual(boundedEvents.find((event) => event.type === "odai/output-budget-applied").data, {
    turn: 1,
    step: 1,
    responsibility: "frontend",
  ...
  const renderedRouting = routingTool.output.render({}, shownRouting)[0].text;
  ...
  assert.match(
    outputTool.output.render({}, shownOutput)[0].text,
