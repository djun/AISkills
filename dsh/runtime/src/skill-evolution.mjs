...
function nonEmptyString(value, field, maximum = Number.POSITIVE_INFINITY) {
...
function utf8Text(buffer, field) {
...
function assertUtf8String(value, field) {
...
function safeGenerationId(value, field = "generationId") {
...
function ensureStoreDirectory(root, name) {
...
function requireStoreDirectory(root, name) {
  ensureStoreRoot(root);
  const path = safePath(root, name);
...
function normalizeHistoryEvent(value, index) {
  ...
  if (!["activate", "rollback", "deactivate"].includes(event.action)) {
...
export function readSkillEvolutionState(root) {
...
export function resolveSkillEvolutionRoot(configuredPath, env = process.env) {
...
export function skillEvolutionDisabled(env = process.env) {
  return env.ODAI_DISABLE_EVOLUTION === "1";
}

function manifestValue(bundle) {
  return {
...
function writeBundleSnapshot(root, snapshot) {
...
function assertEvolvablePath(path, manifest, field = "path") {
...
function normalizeProposedChanges(value, manifest) {
...
function applyExactReplacements(text, replacements, field) {
  let result = text;
...
function mergePatchGroups(parentPatches, proposedChanges, upstreamSnapshot, effectiveSnapshot) {
  const patches = new Map(parentPatches.map((patch) => [patch.path, {
    path: patch.path,
  ...
  for (const change of proposedChanges) {
    ...
    const patch = patches.get(change.path) ?? {
      path: change.path,
      baseSha256: sha256(upstreamSnapshot.files.get(change.path)),
      replacements: [],
    };
...
function normalizePatchMetadata(value, manifest) {
  if (!Array.isArray(value) || value.length === 0 || value.length > 20) {
...
function normalizeGenerationAuthorization(value) {
...
function generationIdentity(metadata) {
  return {
    schemaVersion: STORE_SCHEMA_VERSION,
...
function generationIdFor(metadata) {
  return sha256(Buffer.from(JSON.stringify(generationIdentity(metadata)), "utf8"));
}

function normalizeGenerationMetadata(value, expectedId) {
...
function assertExactBundleTree(root, manifest) {
...
function generationAuthorizationProfile(baseSnapshot, resultSnapshot, patches) {
...
function activationPhrase(generationId, level) {
  return `ACTIVATE${level === "breaking" ? " BREAKING" : ""} ODAI EVOLUTION ${generationId}`;
}

function validateGeneration(root, generationId, ancestry = new Set()) {
  ...
  for (const path of baseBundle.manifest.requiredFiles) {
    ...
    if (!expected.equals(actual)) throw new Error(`Odai evolution generation ${id} has an untracked change in ${path}`);
  }
...
function generationSummary(validated) {
  return Object.freeze({
...
function createGeneration(root, upstreamBundle, finalSnapshot, patches, options = {}) {
...
function appendStateChange(root, state, action, target, evidence, now) {
...
function preserveConflict(root, generation, upstreamBundle, conflicts, authorization, now) {
...
function rebaseGeneration(root, generation, upstreamBundle, authorization, now) {
...
function upstreamSelection(selection) {
...
export function applySkillEvolutionSelection(upstream, root, options = {}) {
...
function requireHumanAuthorization(agent, phrase, action) {
  ...
  if (boundary?.type !== "turn/start" || !Number.isSafeInteger(boundary.data?.turn) || boundary.data.turn < 0) {
...
function actionAuthorizationPhrase(action, current, target) {
...
function proposalAuthorization(objective, expectedBundleDigest, changes) {
...
function assertActionArguments(args, action, allowed) {
...
function activeSummary(root, state, currentUpstream) {
...
function renderEvolutionResult(value) {
  if (value.action === "show") {
    ...
    return `Active Odai evolution ${value.active.generationId}; rebaseRequired=${String(value.active.rebaseRequired)}.`;
  }
  ...
  if (["propose", "rebase"].includes(value.action) && ["candidate", "reused"].includes(value.status)) {
    return `Created inactive Odai evolution candidate ${value.generation.generationId}. Explicit activation is still required.`;
  }
...
export function createSkillEvolutionTool(root, options = {}) {
  const currentSelectionFor = typeof options.currentSelectionFor === "function" ? options.currentSelectionFor : () => options.selection;
  ...
  const currentUpstream = (agent) => upstreamSelection(current(agent));
    ...
    execute(args, execution) {
      ...
      if (args.action === "rebase") {
        ...
        try {
          ...
          let result;
          if (["candidate", "reused"].includes(rebased.status)) {
