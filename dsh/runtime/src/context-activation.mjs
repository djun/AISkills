...
export const ODAI_CORE_TOOL_NAMES = Object.freeze(["odai_context_capability", "odai_responsibility_gap"]);
...
const ROUTING_CONFIG_CUE = /(?:(?:研究|调查|规划|计划|执行|审查|验收|前端|researcher|planner|executor|reviewer|frontend).{0,24}(?:模型|路由|映射|provider|model|route|mapping)|(?:模型|路由|映射|provider|model|route|mapping).{0,24}(?:研究|调查|规划|计划|执行|审查|验收|前端|researcher|planner|executor|reviewer|frontend)|(?:所有|当前|职责|责任)?(?:模型|路由)(?:配置|映射)|routing config)/iu;
...
const OUTPUT_CONFIG_CUE = /(?:(?:输出|回复|回答).{0,20}(?:normal|正常|简洁|精简|concise|economy|经济|token|上限)|(?:maxTokens|token 上限|output mode|输出模式))/iu;
...
export function classifyContextActivation(text) {
...
export function activeOdaiToolNames(activation, options = {}) {
  if (options.child) return Object.freeze([]);
  const names = new Set(ODAI_CORE_TOOL_NAMES);
...
export function inactiveOdaiToolNames(activeNames) {
