...
In auto mode this responsibility is reachable when an active frozen route card proves observable separation benefit and either the original current task already authorizes implementation or the user explicitly continues it. It runs in the current controller turn because DSH routed children are read-only. Follow the claimed card exactly; do not reinterpret the request, broaden scope, or claim final acceptance. The card is consumed only after the effective executor request route is verified; route failure leaves it retryable.`,
