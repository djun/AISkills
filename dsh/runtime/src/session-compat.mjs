...
function isDshCommand(commandLine) {
...
export function listActiveDshProcesses(platform = process.platform) {
...
function assertDshStopped(options) {
  let active;
  try {
...
function transformLine(line) {
  ...
  if (typeof record?.type !== "string" || !record.type.startsWith(ODAI_EVENT_PREFIX) || record.ignorable === true) {
    return { line, repaired: 0 };
  }
  if (!LEGACY_ODAI_AUDIT_EVENT_TYPES.has(record.type)) {
    throw new Error(`refusing unknown Odai session event type ${JSON.stringify(record.type)}; use a compatible Odai repair version`);
  }
  return { line: JSON.stringify({ ...record, ignorable: true }), repaired: 1 };
}

function transformCompleteJsonl(text) {
  const output = [];
  let repaired = 0;
  let offset = 0;
  for (;;) {
...
function compressZstdFrame(plaintext) {
  return zlib.zstdCompressSync(plaintext, {
    params: { [zlib.constants.ZSTD_c_checksumFlag]: 1 },
  });
}

function transformZstd(source) {
  assertZstdAvailable();
  ...
  for (const [index, frame] of frames.entries()) {
    ...
    if (index === 0) {
      ...
      if (!header.endsWith("\n") || JSON.parse(header.trimEnd())?.type !== "session") {
    ...
    repaired += transformed.repaired;
    if (transformed.repaired === 0) {
      output.push(encoded);
      continue;
    }
    const replacementPlaintext = Buffer.from(transformed.text, "utf8");
    const replacement = compressZstdFrame(replacementPlaintext);
    if (!zlib.zstdDecompressSync(replacement).equals(replacementPlaintext)) {
      throw new Error("Zstandard migration verification failed");
    }
    output.push(replacement);
  }
  if (tornStart !== undefined) output.push(source.subarray(tornStart));
  return {
    buffer: repaired > 0 ? Buffer.concat(output) : source,
    repaired,
    torn: tornStart !== undefined,
  };
}

function transformArtifact(path) {
  ...
  return {
    source,
    buffer: transformed.repaired > 0 ? Buffer.from(transformed.text, "utf8") : source,
    repaired: transformed.repaired,
    torn: transformed.torn,
  };
}

function writeDurably(path, content, mode) {
...
function syncDirectory(path) {
  ...
  try {
    ...
    // Windows does not expose directory fsync; the file itself was already synced.
  } finally {
    if (handle !== undefined) closeSync(handle);
  }
}

function backupPathFor(path, source) {
  const digest = createHash("sha256").update(source).digest("hex").slice(0, 16);
  return `${path}.odai-compat-${digest}.bak`;
}

function createVerifiedBackup(path, source) {
  ...
  if (!readFileSync(backupPath).equals(source)) {
    throw new Error(`session backup does not match the migration source: ${backupPath}`);
  }
  return backupPath;
}

function replaceAtomically(path, source, replacement, backupPath) {
  if (!readFileSync(path).equals(source)) {
    throw new Error("session artifact changed while compatibility migration was preparing it; stop every DSH process and retry");
  }
  ...
  try {
    ...
    if (published) {
      ...
      try {
        writeDurably(restore, readFileSync(backupPath), mode);
...
function processLegacySessionLogs(options, writeChanges) {
...
export function inspectLegacySessionLogs(options = {}) {
  return processLegacySessionLogs(options, false);
}

export function repairLegacySessionLogs(options = {}) {
  if (options.confirmDshStopped !== true) {
    throw new Error("refusing to rewrite session artifacts until confirmDshStopped is true; stop every DSH process first");
  }
