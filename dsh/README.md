...
Odai persists only the mapping the user explicitly names. A change applies from the next user turn. Every controller turn receives a merged effective mapping snapshot that supersedes stale conversation summaries; `odai_routing_config show` distinguishes persisted and deployment sources and includes the latest actual route receipt. Configuration is not proof of use. Same-turn upgrades are checked against DSH's effective request header, a mismatch makes the controller read-only, and responsibility children cannot complete without matching route evidence. Generic subagents remain generic unless explicitly labeled `odai-<responsibility> ...`. The Plugin already includes the canonical skill and runtime; the Agent also includes both and does not require the Plugin. Installing both is normally redundant.
...
Less explicit semantic candidates can be submitted by the current controller through `odai_memory consider` with an exact excerpt from that same open turn. They remain `pending` and inert until repeated independent evidence or explicit confirmation. Global and canonical-path-hashed project scopes are isolated; only active, relevant, bounded records are injected as low-priority plugin context. An unresolved conflict suppresses the stale active record instead of silently choosing either side. Correction or confirmation records supersession. Forget and authorized clear physically remove matching content; bulk clear requires an exact preflight phrase, children cannot inspect the store, and an invalid/symlinked store fails closed. Agent install/update/uninstall and Plugin updates never manage or delete this store. Deployment may set `memory.mode: off` as a hard disable that persisted user settings cannot override; otherwise a persisted natural-language mode change takes effect on the next turn.

Human-safety continuity is deliberately separate at `$DSH_HOME/odai/human-safety-continuity.json`. Odai never writes current mood, crisis language, diagnosis, or a risk score automatically. `odai_human_safety_continuity` accepts only an authenticated current direct-user request to save, correct, remove, view, export, or physically clear four bounded categories: care preferences, signals the user explicitly wants noticed, support the user says helps, and user-authored safety-plan steps. Added/replacement text must occur byte-for-byte in that user message; credentials and contact details are rejected. Entries persist until the user removes or physically clears them. New controller sessions can use the record to tailor natural care, but it is historical preference rather than evidence of current risk and is never exposed to children. Current credible self-harm or suicide inclination triggers timely care and a direct safety check even without a plan; plan, means, and action determine escalation urgency, not whether Odai intervenes.
