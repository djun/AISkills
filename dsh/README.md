...
Both published DSH surfaces default routing to `auto` while keeping the governance skill source pinned to `bundled`; neither ships a researcher, planner, executor, reviewer, or frontend model mapping. Ordinary requests stay on the selected controller. A complete contextual or explicit planning gap upgrades that same turn only when the user has configured its responsibility model. Reviewer delegates only from a complete hash-addressed evidence packet; an incomplete packet permits at most a read-only same-turn check without an independence claim. Substantial multi-surface frontend work also upgrades in place, while narrow UI fixes stay direct with a structured skip reason. If a needed responsibility is unconfigured, Odai names it and asks the user for a natural-language provider/model choice. A missing or failed high-impact route makes the controller read-only instead of silently proceeding.
