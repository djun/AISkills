...
Both packages currently accept only `@deepseek-ai/dsh@0.1.1-rc.2`; `0.2.12` does not support rc.7, rc.1, or any other developer-preview release. The Agent package uses the rc.2 Standard composition directly and carries no older-release rendering branch. A DSH version update must refresh this composition and rerun Plugin, Agent, and coexistence probes against every supported release.
